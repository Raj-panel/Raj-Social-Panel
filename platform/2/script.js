...
const servicesData = {
    instagram: {
        ...
        "Repost Lifetime": [
            ...
            { id: 132, name: "500 Reposts", price: 59, badge: "⭐ Popular", badgeClass: "badge-popular", min: 500, max: 500, refill: "Lifetime", drop: "Non-Drop", quality: "Real", speed: "Fast", description: "50 Reposts." },
...
// Safe Element Helper (যাতে কোনো উপাদান না থাকলে জাভাস্ক্রিপ্ট ক্র্যাশ না করে)
...
function switchPlatform(platform) {
    ...
    const tabInsta = document.getElementById('tab-instagram');
...
function getActiveService() {
    const catSelect = document.getElementById('categorySelect');
    const servSelect = document.getElementById('serviceSelect');
    if (!catSelect || !servSelect) return null;

    const cat = catSelect.value;
    const idx = servSelect.value;
...
function updateDetails() {
    ...
    setElementText('dName', s.name);
    setElementText('dRefill', s.refill || 'Lifetime');
    setElementText('dDrop', s.drop || 'Non-Drop');
    setElementText('dQuality', s.quality || 'High Quality');
    setElementText('dSpeed', s.speed || 'Fast');
    setElementText('dDesc', s.description || 'No description available.');

    if (qtyInput) {
        if(s.type === "custom") {
            setElementText('dPrice', `₹${s.pricePer1000.toFixed(2)} per 1K`);
            setElementText('minMaxText', `Min: ${s.min} | Max: ${s.max.toLocaleString()}`);
            qtyInput.readOnly = false;
            qtyInput.value = s.min;
        } else {
            setElementText('dPrice', `₹${s.price.toFixed(2)} (Fixed)`);
            setElementText('minMaxText', `Fixed Package: ${s.min} Quantity`);
            qtyInput.value = s.min;
...
function calculateTotal() {
    ...
    setElementText('totalPrice', `₹${total.toFixed(2)}`);
}

// ===============================================
// PLACE ORDER & REDIRECT TO CHECKOUT PAGE
// ===============================================
function placeOrder() {
...
// Page Load Events Initializer
document.addEventListener('DOMContentLoaded', () => {
    initTheme();
    loadCategories();

    // Event Listeners addition safely
    const catSelect = document.getElementById('categorySelect');
    if (catSelect) {
        catSelect.addEventListener('change', loadServices);
    }

    const servSelect = document.getElementById('serviceSelect');
    if (servSelect) {
        servSelect.addEventListener('change', updateDetails);
    }
